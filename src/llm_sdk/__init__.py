...
class Small_LLM_Model:
    """Lightweight Hugging Face causal-LM wrapper for experimentation.

    Parameters
    ----------
    model_name: str, default="Qwen/Qwen3-0.6B"
        Identifier of the model on the Hub. Feel free to replace it
        with one of the candidates you listed (as soon as they become
        available on the Hub).
    device: str | None, default=None
        Computation device.  If *None* we automatically select ``mps``
        when available on macOS, ``cuda`` when available, otherwise we
        fall back to ``cpu``.
    dtype: torch.dtype | None, default=None
        Numerical precision.  When using a GPU or MPS we default to
        ``float16`` to keep memory usage reasonable; on CPU we keep
    ...
    def encode(self, text: str) -> torch.Tensor:
        """Tokenise *text*; return a 2-D ``input_ids`` tensor on the device."""
    ...
    def get_logits_from_input_ids(self, input_ids: list[int]) -> list[float]:
        """Return raw next-token logits (no softmax) as a list of floats."""
        input_tensor = torch.tensor(
            [input_ids], device=self._device, dtype=torch.long)
        with torch.no_grad():
            out = self._model(input_ids=input_tensor)
        # Last token logits for batch size 1
